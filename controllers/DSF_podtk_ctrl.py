...
"""DSF_podtk_ctrl.py - controller for the POD Toolkit
...
from models.DSF_podtk_model import DSF_PODData
...
class DSF_PODWindowController(object):
    """Controller for the PODWindow UI"""

    def __init__(self, view):
        self.view = view
        self.model = DSF_PODData(self)

    def get_models(self):
        """Retrieves the list of PODModels
        and populates the TreeCtrl"""
        pod_models = self.model.get_models()
        for model in pod_models:
            self.view.modeltree.add_model(model)

    # Event Handlers
    def on_import_data(self, evt):
        """ Handles Import File request"""
        wildcard = "Excel files (*.xls*)|*.xls*"
        style = wx.FD_OPEN | wx.FD_FILE_MUST_EXIST
        file_dlg = wx.FileDialog(self.view, 'Import Data', wildcard = wildcard, style = style)
        if file_dlg.ShowModal() == wx.ID_CANCEL:
            return
        filename = file_dlg.GetPath()
        """ TODO now find out what columns in Sheet1 are size, results
        maybe even give an error message? """
        if self.model.import_data(filename) is False:
            return
        # now fill in grid of data, generate chart
        self.model.plot_input_data(self.view.axes1)
        self.view.Refresh()

    def on_export_data(self, evt):
        """ Handles Export File request"""
        wildcard = "Excel files (*.xlsx)|*.xlsx"
        style = wx.FD_SAVE
        file_dlg = wx.FileDialog(self.view, 'Export Data', wildcard = wildcard, style = style)
        filename = self.model.get_filename()
        file_dlg.SetFilename(filename)
        if file_dlg.ShowModal() == wx.ID_CANCEL:
            return
        filename = file_dlg.GetPath()
        ...
        while True:
            ...
            if not model_thd.is_alive():
                try:
                    ...
                    # NEED TO FIX THIS FOR THE NEW UI
                    model_instance.plot_POD_results(self.view.axes1)
